...
// ---------------------------------------------------------------------------
// GlbCache Implementation
// ---------------------------------------------------------------------------
GlbCache& GlbCache::Get() {
    static GlbCache instance;
    return instance;
}

std::shared_ptr<const CachedGlb> GlbCache::GetOrLoad(const std::string& glbPath) {
    ...
    auto entry = std::make_shared<CachedGlb>();
    entry->path = glbPath;
    ...
    if (gh->id != 0x0630) {
        printf("[GlbCache] GLB PLM magic mismatch (0x%04X) in %s\n", gh->id, glbPath.c_str());
        entry->loaded = false;
        m_cache[glbPath] = entry;
        return entry;
    }
    ...
    entry->loaded = true;
    printf("[GlbCache] Loaded & cached GLB (%d objects, %d textures): %s\n",
...
void GlbCache::Invalidate(const std::string& glbPath) {
    std::lock_guard<std::mutex> lock(m_mutex);
